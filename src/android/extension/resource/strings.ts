    ...
    public afterResources() {
        for (const node of this.application.cacheProcessing) {
            if (node.tagName === 'SELECT') {
                const stored: ObjectMap<string[]> = node.data(ResourceHandler.KEY_NAME, 'optionArray');
                if (stored && !node.hasBit('excludeResource', $enum.NODE_RESOURCE.OPTION_ARRAY)) {
                    ...
                    if (stored.stringArray) {
                        result.push(
                            ...stored.stringArray.map(value => {
                                const name = ResourceHandler.addString($xml.replaceCharacter(value), '', this.options.useNumberAlias);
                    ...
                    if (result.length > 0) {
                        ...
                        let arrayName = '';
                        for (const [storedName, storedResult] of ResourceHandler.STORED.arrays.entries()) {
                        ...
                        if (arrayName === '') {
                            ...
                            ResourceHandler.STORED.arrays.set(arrayName, result);
                        }
                ...
                const stored: NameValue = node.data(ResourceHandler.KEY_NAME, 'valueString');
                if (stored && !node.hasBit('excludeResource', $enum.NODE_RESOURCE.VALUE_STRING)) {
                    ...
                    const name = ResourceHandler.addString(stored.value, stored.name, this.options.useNumberAlias);
